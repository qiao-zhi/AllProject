...
public class CreateSimpleMuban {
	@Test
	public void test1(){
		String name = String.valueOf(System.currentTimeMillis());
		HSSFWorkbook wb = new HSSFWorkbook();
		CreateSimpleMuban.createDanXuan(wb,100,name);
	}
	 ...
	 *@param num 单选题数量
	 */
	public static void createDanXuan(HSSFWorkbook wb,int num,String name) {
		//获取解析xml文件路径
		System.out.println(1);
//		String path = CreateSimpleMuban.class.getClassLoader().getResource("resource/student.xml").getPath();
...
//			下面这个属性可以在模板XML中指定生成几列，也可以通过参数指定模板
//			int repeat = tr.getAttribute("repeat").getIntValue();
			...
			//生成Excel导入模板到本地指定文件夹下面
			File tempFile = new File(name+".xls");
			tempFile.delete();
	...
	private static void setType(HSSFWorkbook wb, HSSFCell cell, Element td) {
		...
		if("NUMERIC".equalsIgnoreCase(type)){
			...
			Attribute strAttr = td.getAttribute("value");
			if(strAttr!=null){
				cell.setCellValue(strAttr.getValue());
			}
