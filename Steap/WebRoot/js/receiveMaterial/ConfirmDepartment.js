						...
						$.ajax({
									...
									success : function(projectsList) {
										...
										for (var i = 0; i < projectsList.length; i++) {
											...
											var a = i - (-1);
											var _tr = "<tr><td>" + a + "</td>";
											_tr += "<td>" + projectsList[i].projectName + "</td>";
											_tr += "<td>" + projectsList[i].projectId + "</td>";
											_tr += "<td>" + projectsList[i].applicant + "</td>";
											_tr += "<td>" + projectsList[i].domain + "</td>";
											_tr += "<td>" + projectsList[i].applicationDate + "</td>";
											_tr += "<td>" + projectsList[i].chargeUnit + "</td>";
											_tr += "<td>" + projectsList[i].chargePerson + "</td>";
											_tr += "<td class='hidden'>" + projectsList[i].distributionDate + "</td>";
											_tr += "<td class='hidden'>" + projectsList[i].status + "</td>";
											_tr += "<td title="+ projectsList[i].status +">" + dic[projectsList[i].status] + "</td>";
											_tr += "<td class='hidden'>" + projectsList[i].remark + "</td>";
											if(projectsList[i].remark == "已确定") {
												_tr += "<td><a href='.bs-example-modal-lg' data-toggle='modal'"
													+"class='operation_view' data-toggle='tooltip'"
													+"data-placement='bottom' title='查看或修改项目信息'>"
													+"<i class='glyphicon glyphicon-search'></i></a>" 
													+"</td></tr>";
											} else if(projectsList[i].remark == "确定负责部门"){
												_tr += "<td><a href='#destribution_modal' data-toggle='modal'"
														...
														+"</td></tr>";
											} else if(projectsList[i].remark == "查看" || projectsList[i].remark == "只查看"){
												_tr += "<td><a href='.bs-example-modal-lg' data-toggle='modal'"
													+"class='operation_view' data-toggle='tooltip'"
...
//					});
					...
					$("#projectTable").on("click",".operation_view",function(){
						...
						var i = $(this).parent().siblings("td:eq(11)").text();
						...
						$("#chargeUnitS").text(f);
						$("#chargePersonS").text(g);
						$("#destributionDateS").text(h);
...
//						searchCharge();
					...
					$("#modifyConfirmB").click(function(){
						//$("#closeBasicModal").trigger("click");
//						$("#openConfirmM").trigger("click");
						$('#basicMessage').modal('hide');
						//$('#destribution_modal').modal('show');
						
						$('#basicMessage').one('hidden.bs.modal', function (e) {
							  // do something...
//							$('#basicMessage').off("click");
							$('#destribution_modal').modal('show');
							})
					})
...
//										addNewLine();
				...
				/*点击关闭确定负责部门时，清空表格和内容*/
				$("#closeConfirmDepart").click(function(){
					/* 清空表格 */
					$("#departmentTable  tr:not(:first)").html("");
					$("#getPerson3").text("");
				});
