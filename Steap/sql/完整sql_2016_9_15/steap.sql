...
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8 COMMENT='项目审核表';
...
INSERT INTO `t_unit_base_info` VALUES ('10001', '综合部', '10', '综合部');
INSERT INTO `t_unit_base_info` VALUES ('10002', '项目评估部', '10', '项目评估部');
INSERT INTO `t_unit_base_info` VALUES ('10003', '成果评估部', '10', '成果评估部');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER  VIEW `v_complete_material_file` AS SELECT
	`complete_material`.`fileId` AS `fileId`,
	`complete_material`.`completeId` AS `completeId`,
	`complete_material`.`projectId` AS `projectId`,
...
FROM
	(
		...
		JOIN `t_files_index` ON (
			(
				`complete_material`.`fileId` = `t_files_index`.`fileId`
			)
		)
	) ;
